dat.mod.before <- filter(dat.mod, period == 'before' & frozen == FALSE)
dat.mod.after <- filter(dat.mod, period == 'after' & frozen == FALSE)
...
for (i in 1:length(responses)) {
  ...
  pdf(paste0('figures/nonfrozen/', site, '_rf_pp_', responses[i], '.pdf'), heigh = 6, width = 6)
  ...
  pdf(paste0('figures/nonfrozen/', site, '_varImp_', responses[i], '.pdf'), heigh = 5, width = 5)

  varImpPlot(mod, type = 1, main = '')
  dev.off()
  ...
  fig.name = paste0('figures/nonfrozen/', site, '_rf_modsum_', responses[i], '.pdf')
...
temp_filename <- file.path('data_cached/nonfrozen', paste0(site, '_before_after_residual_analysis.csv'))
...
temp_filename <- file.path('data_cached/nonfrozen', paste0(site, '_percent_reduction_before_after.csv'))
