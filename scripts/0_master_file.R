...
site <- 'SW1' # site abbreviation that may be used for file naming conventions
...
######## BMP timeline ############
start_date <- as.POSIXct('2012-01-10 00:00:01') # YYYY-MM-DD HH:MM:SS date/time of study start
                             # This can be the first second of the day on the date of the first sampling event
bmp_date <- as.POSIXct('2015-06-01 00:00:01') # YYYY-MM-DD HH:MM:SS date/time of BMP implementation. All events before
...
wq_file <- 'ESW1 storm event data.csv' # file name (with .csv extention) where event-level water quality data are stored
